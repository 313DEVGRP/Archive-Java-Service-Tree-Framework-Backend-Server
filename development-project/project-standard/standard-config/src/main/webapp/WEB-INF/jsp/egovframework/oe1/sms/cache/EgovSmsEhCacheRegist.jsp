...
<%
  /**
  * @Class Name : EgovSmsEhCacheRegist.jsp
  * @Description : EH 캐시 신규 추가  등록 화면
...
function fn_egov_save(){
	...
	if(checkValue()){
	    var options = { 
			   	   success     : responseCtl,
			       url         : "<c:url value='/ole/sms/saveNewEhCache.sms'/>",
			       type        : "post", /* get, post */
...
function responseCtl(html, status){
	...
	if(html.indexOf("EgovSmsEhCacheRegist.jsp") > 0){
		alert("동일한이름의 캐시가 이미 존재하여 추가 할 수 없습니다.");
...
<form:form commandName="ehCacheVO" id="ehCacheVO" name="ehCacheVO">
	<div id="h2_topnav"><h2><strong>EhCache 설정관리 등록</strong></h2></div>
		  	<div id="datail_table">
				...
				<caption>EhCache Config Regist</caption>
				  <tr>
			...
			<div id="btn_style01">
				...
				<a title="초기화" href="/#" onclick="javascript:fn_egov_init();return false;"><span>초기화</span></a>
