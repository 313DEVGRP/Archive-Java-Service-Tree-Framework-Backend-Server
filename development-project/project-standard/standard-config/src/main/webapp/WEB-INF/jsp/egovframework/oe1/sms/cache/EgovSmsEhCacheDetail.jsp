...
<%
  /**
  * @Class Name : EgovSmsEhCacheDetail.jsp
  * @Description : EH캐시 설정 상세 화면
...
<title>EhCache Config Detail</title>
...
function fn_egov_save(){
	...
	if(checkValue()){
	    var options = { 
			   	   success     : responseCtl,
			       url         : "<c:url value='/ole/sms/updateEhCacheService.sms'/>",
			       type        : "post", /* get, post */
...
function fn_egov_delete(){
	...
	var options = { 
		       ...
		       url         : "<c:url value='/ole/sms/deleteEhCacheService.sms'/>",
		       type        : "post", /* get, post */
		       dataType    : "html" /* xml, html, script, json */
		}; 
	jQuery("#ehCacheVO").ajaxSubmit( options );
...
function fn_egov_preview(){
	...
	if(checkValue()){
	    ...
	    frm.method="post";
	    frm.action="<c:url value='/ole/sms/selectEhCachePreview.sms'/>";
	    frm.target="Preview";
	    frm.submit();
	}
...
function checkValue(){
	...
	if(disk_store == ''){
		alert('diskStore Path는 필수 입력사항입니다.');
		return false;
	}
...
<input type="hidden" id="ehCacheNm" name="ehCacheNm" value="<c:out value='${ehCacheVO.ehCacheNm}'/>" />
	<div id="h2_topnav"><h2><strong>EhCache 설정관리 상세</strong></h2></div>
		  	<div id="datail_table">
				<table id="tbl" summary="Ehcache name,diskStore Path,attribute" class="">
				<caption>EhCache Config Detail</caption>
				  <tr>
						<th scope="row">Ehcache name</th>
						<td colspan="2">
							<c:out value='${ehCacheVO.ehCacheNm}'/>
						</td>			
					</tr>
					<tr>
					...
					<tr>
						<th scope="row" width="20%" align="center">diskAccessStripes</th>
						<td>
					...
					<tr>
						<th scope="row" width="20%" align="center">eternal</th>
						<td>
							<label for="eternal">
					...
					<tr>
						<th scope="row" width="20%" align="center">maxElementsInMemory</th>
						<td>
					...
					<tr>
						<th scope="row" width="20%" align="center">memoryStoreEvictionPolicy</th>
					...
					<tr>
						<th scope="row" width="20%" align="center">overflowToDisk</th>
						<td>
					...
					<tr>
						<th scope="row" width="20%" align="center">timeToIdleSeconds</th>
						<td>
