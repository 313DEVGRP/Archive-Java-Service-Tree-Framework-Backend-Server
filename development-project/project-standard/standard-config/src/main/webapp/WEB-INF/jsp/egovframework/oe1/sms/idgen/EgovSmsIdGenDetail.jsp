...
<%
  /**
  * @Class Name : EgovSmsIdGenDetail.jsp
  * @Description : Id Gen 설정 상세 화면
...
<title>ID Gen Config Detail</title>
...
function fn_egov_save(){
	...
	if(checkValue()){
		var options = { 
			   	   success     : responseCtl,   
			       url         : "<c:url value='/ole/sms/updateIdGenServiceBean.sms'/>",
			       type        : "post", /* get, post */
...
function fn_egov_delete(){
	var options = { 
		       ...
		       url         : "<c:url value='/ole/sms/deleteIdGenServiceBean.sms'/>",
		       type        : "post", /* get, post */
		       dataType    : "html" /* xml, html, script, json */
		}; 
...
function fn_egov_preview(){
	if(checkBeforePreviw()){
	    ...
	    frm.method="post";
	    frm.action="<c:url value='/ole/sms/previewIdGenServiceBean.sms'/>";
...
<input type="hidden" name="beanClassNm" id="beanClassNm"  value="<c:out value='${beanVO.beanClassNm}'/>" />
	<div id="h2_topnav"><h2><strong>ID Gen 설정관리 상세</strong></h2></div>
		  	<div id="datail_table">
				<table id="tbl" summary="Bean정보, BeanID, Class명, destroy-method,프로퍼티" >
				<caption>ID Gen Config Detail</caption>
                  <tr>
                  ...
                  <tr>
                    <th colspan="2" width="20%" scope="row">class명</th>
                    <td colspan="4">
                    	<c:out value='${beanVO.beanClassNm}'/>			                    
                    </td>
                  </tr>
                  <tr>
                    <th colspan="2" width="20%" scope="row"><label for="destroyMethodNm">destory-method</label></th>
