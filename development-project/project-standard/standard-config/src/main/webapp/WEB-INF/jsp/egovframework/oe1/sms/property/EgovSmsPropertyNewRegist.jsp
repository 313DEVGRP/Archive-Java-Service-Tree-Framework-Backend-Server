...
<%
  /**
  * @Class Name : EgovSmsPropertyNewRegist.jsp
  * @Description : 프로퍼티 신규 등록 화면
...
function fn_egov_save(){
	...
	if(checkValue()){
		if(confirm("신규 등록의 경우 기존 설정파일이 삭제되고\n현재 프로퍼티 설정으로 신규 생성됩니다.\n저장하시겠습니까?")){
			var options = { 
				   	   success     : responseCtl,
				       url         : "<c:url value='/ole/sms/insertNewProperty.sms'/>",
				       type        : "post", /* get, post */
...
function fn_egov_preview(){
	...
	if(checkBeforePreviw()){
	    ...
	    frm.method="post";
	    frm.action="<c:url value='/ole/sms/previewNewPropertyService.sms'/>";
...
function fn_egov_addRowToTableNew()
{
...
<input type="hidden" name="beanClassNm" id="beanClassNm" value="<c:out value='${propertyVO.beanClassNm}'/>" />
	<div id="h2_topnav"><h2><strong>프로퍼티 설정관리 등록</strong></h2></div>
			...
			<div id="btn_style01">
				<a title="프로퍼티 추가" href="/#" onclick="javascript:fn_egov_addRowToTableNew();return false;"><span>프로퍼티 추가</span></a>
