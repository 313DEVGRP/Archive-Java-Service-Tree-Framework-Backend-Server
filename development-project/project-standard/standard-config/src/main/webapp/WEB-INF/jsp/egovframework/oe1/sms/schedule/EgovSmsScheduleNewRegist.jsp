...
<%
  /**
  * @Class Name : EgovSmsScheduleNewRegist.jsp
  * @Description : 스케줄 설정 신규 등록 화면
...
function fn_egov_save(){
    ...
    if(checkValue()){
    	if(confirm("신규 등록의 경우 기존 설정파일이 삭제되고\n현재 프로퍼티 설정으로 신규 생성됩니다.\n저장하시겠습니까?")){
			var options = { 
				   	   success     : responseCtl,
				       url         : "<c:url value='/ole/sms/insertNewSchedule.sms'/>",
				       type        : "post", /* get, post */
...
function fn_egov_preview(){
	...
	if(checkBeforePreviw()){
	    ...
	    frm.method="post";
	    frm.action="<c:url value='/ole/sms/previewNewScheduleService.sms'/>";
...
<form:form commandName="beanVO" id="beanVO" name="beanVO">
	<div id="h2_topnav"><h2><strong>스케줄 설정관리 등록</strong></h2></div>
		  	<div id="datail_table">
				<table id="tbl" summary="beanID,class명,destory-method,프로퍼티">
				<caption>Schedule Service Config New Regist</caption>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td colspan="8">                    	
                    	<form:input path="destroyMethodNm" size="30" maxlength="100" cssClass="inputlarge" />
                    </td>
                  </tr>
                </table>                	
			</div>
