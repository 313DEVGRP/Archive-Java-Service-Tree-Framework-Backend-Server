...
<!-- 등록폼 시작  -->
<form:form commandName="requestOfferVO" name="requestOfferVO" action="<c:url value='/dam/spe/req/registRequestOffer.do' />" method="post" enctype="multipart/form-data" >
	<%-- 상산 타이틀 --%>
<table width="100%" cellpadding="1" class="table-search" border="0">
<tr>
	<td class="title_left">
		<h1><img src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" style="vertical-align:middle; display:inline-block;" alt="">&nbsp;지식 정보제공/정보요청 <c:if test="${cmd eq 'reply'}">답변</c:if><c:if test="${cmd eq ''}">등록</c:if></h1>
	</td>
	<td width="150px"></td>
</tr>
</table>
...
<tr>
  ...
  <td class="btnBackground" nowrap><input type="submit" value="<spring:message code="button.save" />" onclick="fn_egov_save_RequestOffer(); return false;" class="btnNew" style="height:20px;width:36px;" >
  </td>
